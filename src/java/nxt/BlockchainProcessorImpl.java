...
final class BlockchainProcessorImpl implements BlockchainProcessor {
	...
	private final Runnable blockImporterThread = new Runnable() {
		...
		public void run() {
			try{
						...
						synchronized (BlockchainProcessorImpl.blockCache){
							...
							try {
								 Thread.sleep(1000L);
									...
									logger.debug("Interrupted", ignored);
								}
								break;
						}
						// Clean up cache
				...
				logger.debug("Uncaught exception in blockImporterThread", (Exception) exception);
			}
		}
	};
				...
				if (pushedForkBlocks == 0) {
					for (int i = myPoppedOffBlocks.size() - 1; i >= 0; i--) {
						...
						try {
							...
							logger.debug("Popped off block no longer acceptable: " + block.getJSONObject().toJSONString(), e);
							break;
						}
					}
				} else {
	...
	public void scan(int height) {
		synchronized (blockchain) {
			...
			logger.info("...done at height " + Nxt.getBlockchain().getHeight());
			isScanning = false;
		} // synchronized
	}
